...
def discPlot(name, model, train_sig, train_bkg, test_sig, test_bkg):
    ...
    plt.hist(train_bkg_pred, histtype='stepfilled', color='red', label='Bkg train', bins=50, range=(0,1), normed=True, alpha=0.5)
    plt.hist(train_sig_pred, histtype='stepfilled', color='blue', label='Signal train', bins=50, range=(0,1), normed=True, alpha=0.5)


    plt.errorbar(y=nb, x=bin_centers, yerr=np.sqrt(nb)*.1, fmt='o', color='blue', label='Bkg test')

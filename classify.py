...
def main(args):
    ...
    for _, files in filelist.items():
        for ifile in files:
            ...
            NN_score = array('f', [0.])
            ht_over_leadjetpt = array('f', [0.])
            HT_o_LeadjetPt = ntree.Branch('HToverLeadJet_pt', ht_over_leadjetpt, 'HToverLeadJet_pt/F')
            disc_branch = ntree.Branch('NN_disc', NN_score, 'NN_disc/F')
            evt_index = 0
            ...
            for _ in range(len(out)): #looping only over the indices in input df
                ...
                NN_score[0] = getGuess(out, out.index[evt_index])
                ht_over_leadjetpt[0] = out.loc[out.index[evt_index], 'HToverLeadJetPt']
                #print (evt_index, out.index[evt_index])
                fout.cd()      
                disc_branch.Fill()
                HT_o_LeadjetPt.Fill()
                evt_index += 1
